	...
	login() {
		...
		if (!!code) {
			axios({
				...
				data: Object.assign({}, this.ajaxConfig.data, {
					code,
					app_id: this.appId
				})
			}).then(res => {
			...
			window.location.href = `https://open.feishu.cn/open-apis/authen/v1/index?redirect_uri=${encodeURIComponent(this.redirectUrl)}&app_id=${this.appId}&state=${JSON.stringify(state)}`
		}
	}
}
